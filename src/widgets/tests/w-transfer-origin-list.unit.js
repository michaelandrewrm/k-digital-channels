...
import Component from '@widgets/w-transfer-origin-list.vue';
...
describe('w-transfer-origin-list.vue', () => {
	let store;
	...
	const moveMoneyOK = {
		namespaced: true,
		actions: { getOrigins: jest.fn().mockResolvedValue(fixture) },
	};

	const moveMoneyKO = {
		namespaced: true,
		actions: { getOrigins: jest.fn().mockRejectedValue() },
	};
	...
	it('has a name equal to w-transfer-origin-list', () => {
		const wp = shallowMount(Component, { localVue, store });

		expect(wp.vm.$options.name).toBe('w-transfer-origin-list');
	});
