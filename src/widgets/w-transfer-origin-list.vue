<template>
	<div class="w-transfer-origin-list" :class="{ '--selected': selected }">
		<transition name="placeholder" mode="out-in">
			<c-placeholder-movement v-if="loading && !fetched" key="placeholder" data-testid="loading" />

			<div
				class="w-transfer-origin-list__main w-transfer-origin-list__list-error"
				v-else-if="error"
				data-testid="error"
			>
				<c-icon class="w-transfer-origin-list__list-error-icon" src="@icons/modalExclamation" />

				<p class="text-m-book w-transfer-origin-list__list-error-text">
					{{ $t('RESOURCE.PRODUCT.ERROR') }}
					<br />
					<br />
					<a href="#" class="w-transfer-origin-list__list-error-link" @click.prevent="refresh">
						{{ $t('RETRY') }}
					</a>
				</p>
			</div>
			...
			<div
				class="w-transfer-origin-list__main w-transfer-origin-list__list"
				v-else-if="fetched"
				key="resources"
				data-testid="list"
				role="list"
			>
				<button
					...
					class="w-transfer-origin-list__button"
					@click="selectProduct(product)"
				>
					<c-card-item
						class="w-transfer-origin-list__card"
						:icon="product.icon"
						:title="product.alias"
...
export default {
	...
	computed: {
		viewProducts({ products }) {
			...
			return products.map((product) => {
				const { id, alias, productNumber } = product;
				const type = product.productFamily;
				const namedType = subtypesById[product.productSubtype.id]?.toUpperCase();
				const balance = product.postedBalance;
				const icon = iconsByFamily[type] || /* istanbul ignore next */ iconsByFamily.account;
				const info = this.$tc(`MY_PRODUCT.${namedType}.AVAILABLE_BALANCE`, 1);
	...
	methods: {
		...
		fetch() {
			...
			return dispatch('move-money/getOrigins')
				.then((data) => {
					this.products = data;
				})
				.catch(() => {
...
.w-transfer-origin-list {
	color: RGB(var(--color-text-primary));
	display: flex;
	flex-direction: column;
}

.w-transfer-origin-list.--selected .w-transfer-origin-list__card {
	animation: dissapear 200ms ease forwards;
}

.w-transfer-origin-list.--selected .w-transfer-origin-list__card[selected] {
...
.w-transfer-origin-list__button {
	appearance: none;
...
.w-transfer-origin-list__list {
	display: grid;
	grid-template-columns: repeat(auto-fill, minmax(320px, 1fr));
	grid-gap: 10px;
}

.w-transfer-origin-list__main {
	flex-grow: 1;
}

.w-transfer-origin-list__main.placeholder-leave-active {
	transform: translateY(0);
	transition: opacity 0s ease-in-out;
	transition-delay: 0ms;
}

.w-transfer-origin-list__list-error {
	text-align: center;
	margin: 10px 0;
}

.w-transfer-origin-list__list-error-icon {
	margin-bottom: 10px;
	color: RGB(var(--color-accent-error));
	font-size: 30px;
}

.w-transfer-origin-list__list-error-link {
