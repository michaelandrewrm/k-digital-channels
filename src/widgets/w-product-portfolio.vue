<template>
	<div class="w-product-portfolio">
		<transition name="placeholder" mode="out-in">
			<component
				class="w-product-portfolio__list"
				:is="widget"
				v-bind="$attrs"
				v-on="$listeners"
				...
				v-if="source && !loading"
			/>
			<div class="w-product-portfolio__error" v-if="error" data-testid="position-detail-error">
				<c-icon class="w-product-portfolio__error-icon" src="@icons/modalExclamation" />

				<p class="text-m-book w-product-portfolio__error-text">
					{{ $t('MOVEMENT.DETAIL_ERROR') }}
					<a href="#" class="w-product-portfolio__error-link" @click="fetch">
						{{ $t('RETRY') }}
					</a>
				</p>
			</div>
...
.w-product-portfolio__error {
	text-align: center;
	margin: 10px 0;
}

.w-product-portfolio__error-icon {
	margin-bottom: 10px;
	color: RGB(var(--color-accent-error));
	font-size: 30px;
}

.w-product-portfolio__error-link {
...
.w-product-portfolio__list.placeholder-leave-active {
